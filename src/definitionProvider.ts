...
export class FoxProDefinitionProvider implements vscode.DefinitionProvider {
     ...
     */
    private async findClassDefinition(className: string, type: string, document: vscode.TextDocument, filename?: string): Promise<vscode.Location[]> {
     ...
     */
    private async searchInSpecificFile(basePath: string, filename: string, className: string): Promise<vscode.Location | null> {
     ...
     */
    private async findComponentDefinitions(basePath: string, className: string): Promise<vscode.Location[]> {
        const locations: vscode.Location[] = [];
     ...
     */
    private async findProductDefinitions(basePath: string, className: string): Promise<vscode.Location[]> {
        const locations: vscode.Location[] = [];
     ...
     */
    private async findStandardClassDefinition(basePath: string, className: string, document: vscode.TextDocument): Promise<vscode.Location[]> {
        const locations: vscode.Location[] = [];
     ...
     */
    private async searchInFile(filePath: string, className: string): Promise<vscode.Location | null> {
        try {
    ...
    private async findFileInWorkspace(basePath: string, fileName: string): Promise<string | null> {
        try {
            const files = await vscode.workspace.findFiles(
                new vscode.RelativePattern(basePath, `**/${fileName}`),
                '**/node_modules/**',
                10
            );
